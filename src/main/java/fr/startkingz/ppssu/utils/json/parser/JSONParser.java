...
public class JSONParser
{
	...
	public JSONObject parse(final File file)
	{
		final StringBuilder JSON_STRING = new StringBuilder();
		try (final Scanner SCANNER = new Scanner(new FileInputStream(file)))
		{
			...
			return (JSONObject) new JSONParser().parse(JSON_STRING.toString());
		} catch (IOException | ParseException e)
		{
			System.err.println("[JSONParser] `" + file.getPath() + "` file unfound, unreachable or incorrect.");
			e.printStackTrace();
			System.exit(0);
		}
	 ...
	 * @param  isResume       - Indicates if it continues previous parsing
	 *                            operation.
	 *                            If set to true, resume parsing the old stream, and
	 *                            parameter 'in' will be ignored. If this method is
	 *                            called for the first time in this instance,
	 *                            isResume
	 *                            will be ignored.
	 * 
	 * @throws IOException
